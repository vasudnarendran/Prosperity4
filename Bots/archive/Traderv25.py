...
# EMERALDS: V20_1 verbatim (proven ~2200 score)
...
# TOMATOES: V20_1 base + ML regime override
# Key insight from offline analysis:
#   - V20_1 trend classifier fires 0 times on local data (too many simultaneous conditions)
#   - Pre-trained logistic regression fires 3.3% of time with 95.9% directional precision
#   - When logit prob > 0.60 → price goes up 5 steps with 95.9% accuracy (+3.2 avg move)
#   - When logit prob < 0.20 → price goes down 5 steps with 97.6% accuracy (-2.7 avg move)
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_HOLD_EXIT_BONUS": 0.65,
    "STRONG_TREND_HOLD_EXIT_BONUS": 0.90,
    "TREND_PASSIVE_PUSH": 0.0,
    "TREND_PASSIVE_SIZE_BONUS": 3.0,
    # ML thresholds — tune these to control aggressiveness
    "LOGIT_UP_THRESHOLD": 0.60,     # prob > this → force trend_up
    "LOGIT_DOWN_THRESHOLD": 0.20,   # prob < this → force trend_down
}
...
class TomatoesTrader(BaseProductTrader):
    """
    V20_1 TOMATOES + ML regime override.

    Pre-trained logistic regression weights (trained on 19987 TOMATOES samples):
        Features: [pred_edge, momentum, imbalance, fit_quality, micro_dev, short_mom]
        Accuracy: 60.2% overall; 95.9% precision when prob > 0.60 (662 calls/day-equivalent)
        V20_1 built-in classifier fires 0 times on this data — logit replaces it.

    Key offline findings:
        Q5 (prob 0.8-1.0): 366 samples, 98.4% correct, avg 5-step move +3.25
        Q4 (prob 0.6-0.8): 296 samples, 92.9% correct, avg 5-step move +3.23
        Q1 (prob 0.0-0.2): 544 samples, 97.6% DOWN,    avg 5-step move -2.74
    """
    PARAMETER_DEFAULTS = DEFAULT_TOMATOES_PARAMS

    # Baked logistic weights — raw (un-normalised) form
    # Score = sum(W[i] * feature[i]) + BIAS; prob = sigmoid(score)
    ...
    def logit_prob(
        ...
        """
        P(price_up_5_steps) from pre-trained logistic regression.
        Features: [pred_edge, momentum, imbalance, fit_quality, micro_dev, short_mom]
        """
    ...
    def classify_state(
        ...
        # Volatile always takes priority
        if float(self.spread) >= self.TOXIC_SPREAD_THRESHOLD and volatility >= self.TOXIC_VOLATILITY_THRESHOLD:
            return "volatile"

        # ML logistic override — fires with 95.9% / 97.6% precision
        if logit_prob > self.LOGIT_UP_THRESHOLD:
            return "trend_up"
        if logit_prob < self.LOGIT_DOWN_THRESHOLD:
            return "trend_down"

        # V20_1 fallback classifier (rarely fires, but kept as safety net)
        if (
    ...
    def run(self) -> List[Order]:
        ...
        # Compute ML logistic probability — primary regime signal
        prob = self.logit_prob(predicted_edge, fit_quality)

        regime = self.classify_state(predicted_edge, fit_quality, volatility, prob)
        ...
        took_buy, took_sell = self.take_orders(
            regime, target_position, adjusted_fair, predicted_edge, fit_quality, volatility,
        )

        buy_quote, sell_quote = self.passive_quotes(
            adjusted_fair, regime, target_position, predicted_edge, fit_quality, volatility,
        )
        position = self.projected_position()
