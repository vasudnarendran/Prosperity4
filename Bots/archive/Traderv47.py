...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "RISK_REDUCE_BUY_SIZE_CAP": 8,
    "RISK_REDUCE_SELL_SIZE_CAP": 4,
    "RISK_REDUCE_SLOW_EDGE_THRESHOLD": 2.35,
    "RISK_REDUCE_SLOW_FIT_THRESHOLD": 0.34,
    "RISK_REDUCE_SLOW_DEPTH_THRESHOLD": 0.65,
    "RANGE_UNWIND_START": 18,
    "RANGE_UNWIND_STRONG": 30,
    "RANGE_UNWIND_ENDGAME_TS": 185000,
    "RANGE_SELL_QUOTE_SHIFT": 1,
    "RANGE_BUY_DEEMPHASIS": 0,
    "RANGE_SELL_SIZE_BONUS": 1,
    "RANGE_PASSIVE_BUY_BUFFER": 80,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def passive_quotes(
        ...
        if regime == "range":
            excess_long = max(0, position - target_position)
            late_unwind = int(getattr(self.state, "timestamp", 0)) >= int(self.RANGE_UNWIND_ENDGAME_TS)
            if excess_long >= int(self.RANGE_UNWIND_START) and (
                late_unwind or excess_long >= int(self.RANGE_UNWIND_STRONG)
            ):
                if sell_quote is not None:
                    shift = int(self.RANGE_SELL_QUOTE_SHIFT)
                    if excess_long >= int(self.RANGE_UNWIND_STRONG):
                        shift += 1
                    sell_quote = max(int(self.best_bid) + 1, sell_quote - shift)
                if buy_quote is not None and excess_long >= int(self.RANGE_UNWIND_STRONG):
                    buy_quote -= int(self.RANGE_BUY_DEEMPHASIS)
    ...
    def take_orders(
        ...
        if (
            ...
            if gate_allowed:
                ...
                if risk_reducing and regime == "range":
                    quantity = min(quantity, int(self.RISK_REDUCE_BUY_SIZE_CAP))
                if aligned_entry:
        ...
        if (
            ...
            if gate_allowed:
                ...
                if risk_reducing and regime == "range":
                    quantity = min(quantity, int(self.RISK_REDUCE_SELL_SIZE_CAP))
                if aligned_entry:
                    quantity = min(quantity, int(self.ALIGNED_TAKE_SIZE_CAP))
                if regime == "range" or risk_reducing or aligned_entry:
                    quantity = min(quantity, target_limit)
    ...
    def run(self) -> List[Order]:
        ...
        if (
            ...
            long_reentry_buffer = target_position + int(self.RANGE_PASSIVE_BUY_BUFFER)
            if (regime == "range" and position <= long_reentry_buffer) or position < target_position:
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                ...
                if (
                    ...
                    and int(getattr(self.state, "timestamp", 0)) >= int(self.RANGE_UNWIND_ENDGAME_TS)
                    and position - target_position >= int(self.RANGE_UNWIND_START)
                ):
                    quantity = min(quantity + int(self.RANGE_SELL_SIZE_BONUS), self.sell_capacity)
                if regime != "range":
