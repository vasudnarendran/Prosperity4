...
# EMERALDS: partner's V20_1 — clear_orders + tiered-take + join/improve quoting (proven ~2200)
...
# TOMATOES: V20_1 base with targeted improvements
DEFAULT_TOMATOES_PARAMS = {
    ...
    "BASE_TAKE_EDGE": 1.00,             # v24: 1.10→1.00 — slightly more aggressive entries
    ...
    "STRONG_TREND_EDGE": 2.00,          # v24: 2.50→2.00 — activate (22,44) bands sooner
    "FIT_THRESHOLD": 0.40,              # v24: 0.45→0.40 — catch slightly more valid trends
    ...
    "TREND_HOLD_EXIT_BONUS": 0.65,      # v24: 0.55→0.65 — ride trends slightly longer
    "STRONG_TREND_HOLD_EXIT_BONUS": 0.90,
    "TREND_PASSIVE_PUSH": 0.0,
    "TREND_PASSIVE_SIZE_BONUS": 3.0,    # v24: 2.0→3.0 — more passive size when trending
}
...
class EmeraldsTrader(BaseProductTrader):
    """
    Partner's V20_1 EMERALDS logic verbatim.
    Key mechanisms: tiered take sizing, clear_orders (inventory flush at fair),
    join/improve passive quoting.
    """
    ...
    def clear_orders(self, adjusted_fair: float) -> Tuple[bool, bool]:
        """Flush inventory at fair value — fires even without edge, 2x more fills."""
        cleared_buy = False
        cleared_sell = False
        position = self.projected_position()
    ...
    def passive_quotes(self, adjusted_fair: float) -> Tuple[Optional[int], Optional[int]]:
        """Join best level or improve by 1 — better queue position than fixed-edge quoting."""
        asks_above_fair = [
...
class TomatoesTrader(BaseProductTrader):
    """
    V20_1 TOMATOES base with targeted improvements:
    - STRONG_TREND_EDGE 2.50→2.00: (22,44) bands activate sooner
    - FIT_THRESHOLD 0.45→0.40: catch slightly more valid trends
    - BASE_TAKE_EDGE 1.10→1.00: more aggressive trend entries
    - TREND_HOLD_EXIT_BONUS 0.55→0.65: ride trends slightly longer
    - TREND_PASSIVE_SIZE_BONUS 2.0→3.0: more passive volume when trending
    """
