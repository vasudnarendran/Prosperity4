...
DEFAULT_TOMATOES_PARAMS = {
   "MID_WEIGHT": 0.25,
    ...
    "HOLD_TIME_COEF": 0.08,
    "HOLD_VOL_COEF": 0.0,
    "RANGE_TARGET_SCALE": 7.0,
    "TREND_TARGET_BASE": 10.0,
    "TREND_TARGET_SCALE": 10.0,
    "VOLATILE_TARGET_SCALE": 2.0,
    "VOLATILE_TARGET_CAP": 6.0,
    "TARGET_DEADBAND": 2.0,
}
...
class BaseProductTrader:
    ...
    def sweep_buy_levels(self, max_price: float, max_qty: int) -> int:
        remaining = min(max(0, int(max_qty)), self.buy_capacity)
        filled = 0
        ...
        for price, volume in self.sell_levels:
            ...
            qty = min(int(volume), remaining)
            if qty <= 0:
                continue

            before = self.buy_capacity
            self.add_buy(int(price), qty)
            executed = before - self.buy_capacity
            filled += executed
            remaining -= executed
    ...
    def sweep_sell_levels(self, min_price: float, max_qty: int) -> int:
        remaining = min(max(0, int(max_qty)), self.sell_capacity)
        filled = 0
        ...
        for price, volume in self.buy_levels:
            ...
            self.add_sell(int(price), qty)
            executed = before - self.sell_capacity
            filled += executed
            remaining -= executed
...
class EmeraldsTrader(BaseProductTrader):
    ...
    def take_orders(self, adjusted_fair: float) -> Tuple[bool, bool]:
        ...
        if buy_take_size > 0 and self.buy_capacity > 0:
            max_buy_price = adjusted_fair
            buy_filled = self.sweep_buy_levels(max_buy_price, buy_take_size)

        if sell_take_size > 0 and self.sell_capacity > 0:
            min_sell_price = adjusted_fair
            sell_filled = self.sweep_sell_levels(min_sell_price, sell_take_size)

        return buy_filled > 0, sell_filled > 0
...
class TomatoesTrader(BaseProductTrader):
    ...
    def controlled_fair_value(
        ...
        base_fair = self.adjusted_fair_value(regime, target_position, predicted_now, predicted_next)
        return base_fair - self.reservation_adjustment(regime, target_position, predicted_edge, volatility)
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        signal = predicted_edge * max(0.40, fit_quality)

        if regime == "volatile":
            raw_target = self.VOLATILE_TARGET_SCALE * signal
            cap = self.VOLATILE_TARGET_CAP

        elif regime == "range":
            raw_target = self.RANGE_TARGET_SCALE * signal
            cap = 14.0

        else:
            direction = 1.0 if regime == "trend_up" else -1.0
            raw_target = (direction * self.TREND_TARGET_BASE) + (self.TREND_TARGET_SCALE * signal)
            cap = 44.0

        clipped = max(-cap, min(cap, raw_target))
        target = int(round(clipped))
    ...
    def take_orders(
        ...
        start_position = self.projected_position()

        buy_edge = self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility)
        sell_edge = self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility)

        max_buy_price = adjusted_fair - buy_edge
        min_sell_price = adjusted_fair + sell_edge
        ...
        if self.buy_capacity > 0:
            buy_limit = self.MAX_TAKE_SIZE + (int(self.TREND_ENTRY_TAKE_BONUS) if regime == "trend_up" else 0)

            if regime != "range":
                if start_position < target_position:
                    buy_limit = min(buy_limit, max(1, target_position - start_position))
                else:
                    buy_limit = 0

            if buy_limit > 0 and int(self.best_ask) <= max_buy_price:
                buy_filled = self.sweep_buy_levels(max_buy_price, buy_limit)
                took_buy = buy_filled > 0

        # SELL sweep
        current_position = self.projected_position()
        if self.sell_capacity > 0:
            sell_limit = self.MAX_TAKE_SIZE + (int(self.TREND_ENTRY_TAKE_BONUS) if regime == "trend_down" else 0)
            ...
            min_sell_price_with_hold = min_sell_price + required_bonus

            if regime != "range":
                if current_position > target_position:
                    sell_limit = min(sell_limit, max(1, current_position - target_position))
                else:
                    sell_limit = 0

            if sell_limit > 0 and int(self.best_bid) >= min_sell_price_with_hold:
                sell_filled = self.sweep_sell_levels(min_sell_price_with_hold, sell_limit)
                took_sell = sell_filled > 0
    ...
    def run(self) -> List[Order]:
        ...
        adjusted_fair = self.controlled_fair_value(
            regime,
            target_position,
            predicted_now,
            predicted_next,
