...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "SPREAD_VOL_COEF": 0.1,
    # Fix 4: SPREAD_INV_COEF now operates on position normalised to [0,1] by position_limit.
    # Old formula: coef * gamma * position  → hit MAX_QUOTE_EDGE cap at position ~9.
    # New formula: coef * gamma * (position / position_limit) → gradient preserved to pos=80.
    # Value changed from 1.1081637 to 4.0 to keep meaningful edge contribution at full position.
    "SPREAD_INV_COEF": 4.0,
    ...
    # Fix 6: CONFLICT_ALPHA_DAMP split into two separate weights.
    # Old: both regression-conflict and imbalance-conflict used the same 0.45 multiplier,
    # compounding to 0.45*0.45=0.20 when both triggered simultaneously (too aggressive).
    # New: regression conflict gets a heavier damp (0.45); imbalance conflict is lighter (0.70),
    # since the two conditions are often correlated and should not fully double-penalise.
    "REGRESSION_CONFLICT_DAMP": 0.45,
    "IMBALANCE_CONFLICT_DAMP": 0.70,
    "MOMENTUM_ALPHA_DAMP": 0.70,
    "POSITION_ALPHA_DAMP_START": 14.0,
    "POSITION_ALPHA_DAMP_END": 28.0,
    # Fix 5: MOMENTUM_TREND_THRESHOLD added as a tunable parameter.
    # Was hardcoded as 0.75 in four places inside classify_state(); cannot be tuned externally.
    ...
    # Fix 1: BREAKOUT_QUOTE_TIGHTEN restored from 0.18 to 0.40.
    # At 0.18: int(round(min(1.0, 2.5*0.18))) = int(round(0.45)) = 0 → shift never fires.
    # At 0.40: fires when breakout_score >= 1.25 → meaningful quote adjustment during breakouts.
...
class TomatoesTrader(BaseProductTrader):
    ...
    def store_flow_metrics(self, flow_metrics: Dict[str, float]) -> None:
        ...
        # Fix 3: removed price_pressure_history — was written every tick but never read anywhere.
    ...
    def hybrid_alpha(self) -> Tuple[float, float]:
        ...
        half_spread = max(1.0, float(self.spread) / 2.0)
        # Fix 2: use smoothed flow bias from history instead of raw current-tick imbalance.
        # bias_history stores the per-tick flow_bias (signed_flow / activity), which is less
        # noisy than self.imbalance and properly reflects book microstructure direction.
    ...
    def guarded_hybrid_alpha(self, hybrid_alpha: float, regression_edge: float, regime: str) -> float:
        ...
        # Fix 6: separate damp weights for each conflict type.
        # Old code applied the same 0.45 multiplier for both, compounding to 0.20 when both
        # triggered — overly aggressive since the two conditions are often correlated.
        if hybrid_alpha * regression_edge < 0:
            weight *= self.REGRESSION_CONFLICT_DAMP  # 0.45 — strong penalty for regression conflict

        if hybrid_alpha * self.imbalance < 0:
            weight *= self.IMBALANCE_CONFLICT_DAMP   # 0.70 — lighter penalty for imbalance conflict
    ...
    def classify_state(
        ...
        if (
            ...
            return "volatile"
        # Fix 5: use MOMENTUM_TREND_THRESHOLD parameter instead of hardcoded 0.75 / -0.25 / 0.25.
        # Breakout-triggered conditions use a relaxed guard (negative of threshold),
        # allowing breakout trend to fire even with slightly adverse momentum.
        momentum_threshold = self.MOMENTUM_TREND_THRESHOLD
        if (
            breakout_score >= 1.0
    ...
    def quote_edge(
        ...
        # Fix 4: normalise position to [0, 1] range before scaling.
        # Old formula multiplied raw position (0–80) directly, exceeding MAX_QUOTE_EDGE at pos ~9.
        # New formula keeps the full gradient meaningful across the entire position range.
        normalized_position = abs(self.projected_position()) / max(1.0, float(self.position_limit))
        edge += self.SPREAD_INV_COEF * gamma * normalized_position
