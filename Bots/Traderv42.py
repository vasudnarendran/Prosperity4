...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "DEPTH_LEVELS_LONG": 5,
    "DEPTH_ALPHA_WEIGHT": 0.22,
    "CHANNEL_WINDOW": 18,
    "CHANNEL_POSITION_WEIGHT": 0.18,
    "CHANNEL_BREAKOUT_BONUS": 0.35,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def breakout_score(
        ...
        if pressure_bias * direction > 0:
            ...
            conviction *= 0.75
        half_spread = max(1.0, float(self.spread) / 2.0)
        depth_direction = depth_alpha / half_spread
        if depth_direction * direction > 0:
            conviction += 0.18 * min(1.5, abs(depth_direction))
        elif depth_direction * direction < 0:
            conviction *= 0.85
        channel_direction = float(channel_state.get("direction", 0.0))
        channel_position = abs(float(channel_state.get("position", 0.0)))
        if channel_direction == direction:
            conviction += self.CHANNEL_BREAKOUT_BONUS * max(0.5, channel_position)
        elif channel_direction == -direction:
            conviction *= 0.70
    ...
    def hybrid_alpha(self) -> Tuple[float, float]:
        ...
        hybrid_fair += self.DEPTH_ALPHA_WEIGHT * depth_alpha
    ...
    def classify_state(
        ...
        if (
            ...
            and (
                ...
                or depth_direction > 0.10
                or channel_direction > 0
            )
        ):
            return "trend_up"
        if (
    ...
    def fair_value(
        ...
        if regime == "range":
            ...
            if channel_direction != 0.0 and channel_direction * channel_position > 0:
                fair += channel_adjustment
            else:
                fair += 0.40 * channel_adjustment
        return fair
    ...
    def run(self) -> List[Order]:
        ...
        provisional_edge = (
            ...
            + (0.20 * pressure_bias)
            + (0.35 * self.DEPTH_ALPHA_WEIGHT * depth_alpha)
        )
