...
# TOMATOES: V25_1 offline-model params + our V25 improvements
# V25_1 additions: OFFLINE_* blending params
# Our V25 improvements: BASE_TAKE_EDGE 1.10→1.00, STRONG_TREND_EDGE 2.50→2.00,
#   FIT_THRESHOLD 0.45→0.40, TREND_HOLD_EXIT_BONUS 0.55→0.65, TREND_PASSIVE_SIZE_BONUS 2.0→3.0
# New: LOGIT_UP_THRESHOLD / LOGIT_DOWN_THRESHOLD for logistic regime override
DEFAULT_TOMATOES_PARAMS = {
    ...
    # V25_1 offline-model blending params (verbatim)
    "OFFLINE_SHORT_BLEND": 0.80,
    "OFFLINE_LONG_BLEND": 0.45,
    "OFFLINE_TARGET_BLEND": 0.35,
    "OFFLINE_FIT_BOOST": 0.10,
    "OFFLINE_HOLD_BONUS": 0.35,
    # V26 logistic thresholds
    "LOGIT_UP_THRESHOLD": 0.60,
    "LOGIT_DOWN_THRESHOLD": 0.20,
}

# V25_1 offline models — trained on 13 features (richer than our 6-feature logit)
...
class BaseProductTrader:
    """V25_1 base — tracks spread, bid, ask history in addition to mid."""
    HISTORY_LENGTH = 8
...
class TomatoesTrader(BaseProductTrader):
    """
    V26 hybrid TOMATOES:
    - V25_1 foundation: 13-feature offline models, richer history, blended predicted_edge,
      effective_fit boosting, target_position +4 bonus on strong offline agreement
    - Our V25 additions: 6-feature logistic classify_state override (prob>0.60/prob<0.20),
      fires independently of V25_1 classifier with 95.9%/97.6% precision
    - Our V25 param improvements: lower BASE_TAKE_EDGE, STRONG_TREND_EDGE,
      FIT_THRESHOLD, higher TREND_HOLD_EXIT_BONUS, TREND_PASSIVE_SIZE_BONUS
    """
    PARAMETER_DEFAULTS = DEFAULT_TOMATOES_PARAMS

    # Our V25 pre-trained logistic weights (6 features, trained on 19987 samples)
    # Features: [pred_edge, momentum, imbalance, fit_quality, micro_dev, short_mom]
    # Overall accuracy: 60.2%; precision when prob>0.60: 95.9%; prob<0.20: 97.6%
    _LOGIT_W = [0.049821, -0.173430, 0.490218, -0.077356, 1.028125, -0.079906]
    _LOGIT_B = -0.306937
    ...
    def regression_metrics(self) -> Tuple[float, float, float, float, float]:
        ...
        cov_xy = sum((i - x_mean) * (p - y_mean) for i, p in enumerate(window))
        ...
        ss_tot = sum((p - y_mean) ** 2 for p in window)
        ss_res = sum((p - f) ** 2 for p, f in zip(window, fitted))
        fit_quality = 0.0 if ss_tot <= 1e-9 else max(0.0, min(1.0, 1.0 - (ss_res / ss_tot)))
        diffs = [abs(window[i] - window[i - 1]) for i in range(1, n)]
        volatility = sum(diffs) / len(diffs) if diffs else 0.0
        return slope, predicted_now, predicted_next, fit_quality, volatility

    # ── Our V25 logistic signal ───────────────────────────────────────────────

    def logit_prob(self, predicted_edge: float, fit_quality: float) -> float:
        """P(price_up_5_steps) from pre-trained 6-feature logistic regression."""
        momentum = float(self.mid) - float(self.recent_average)
        micro_dev = float(self.micro) - float(self.mid)
        short_mom = float(self.mid) - float(self.short_average)
        feats = [predicted_edge, momentum, self.imbalance, fit_quality, micro_dev, short_mom]
        score = sum(self._LOGIT_W[j] * feats[j] for j in range(6)) + self._LOGIT_B
        return 1.0 / (1.0 + math.exp(-max(-30.0, min(30.0, score))))

    # ── Regime classification: logit override → V25_1 fallback ───────────────
    ...
    # ── Remaining TOMATOES methods (V25_1 verbatim) ───────────────────────────
    ...
    def adjusted_fair_value(
        ...
        return self.fair_value(regime, target_position, predicted_now, predicted_next) - (
    ...
    def run(self) -> List[Order]:
        ...
        # V25_1: blended predicted_edge and boosted effective_fit
        ...
        effective_fit = min(
            1.0,
            fit_quality + (self.OFFLINE_FIT_BOOST * min(1.0, abs(offline_short_edge) + 0.5 * abs(offline_long_edge))),
        )
        blended_next = float(self.mid) + predicted_edge

        # V26: when logit agrees with blended direction and is high-confidence,
        # boost effective_fit AND relax the imbalance requirement via a temp override
        prob = self.logit_prob(regression_edge, fit_quality)
        logit_agrees = (prob > self.LOGIT_UP_THRESHOLD and predicted_edge > 0) or \
                       (prob < self.LOGIT_DOWN_THRESHOLD and predicted_edge < 0)
        if logit_agrees:
            effective_fit = min(1.0, effective_fit + 0.20)
            # Temporarily lower imbalance gate: logit micro_dev feature captures same signal
            saved_imb_thresh = self.TREND_IMBALANCE_THRESHOLD
            self.TREND_IMBALANCE_THRESHOLD = 0.05
        else:
            saved_imb_thresh = None

        regime = self.classify_state(predicted_edge, effective_fit, volatility)

        if saved_imb_thresh is not None:
            self.TREND_IMBALANCE_THRESHOLD = saved_imb_thresh
        target_position = self.target_position(regime, target_edge, effective_fit)

        # V25_1: +4 bonus when offline long model strongly confirms trend direction
        ...
        took_buy, took_sell = self.take_orders(
            regime, target_position, adjusted_fair, predicted_edge, effective_fit, volatility,
        )

        # V25_1: loosen sell edge when riding strong uptrend
        if regime == "trend_up" and offline_long_edge > self.TREND_EDGE_THRESHOLD:
        ...
        buy_quote, sell_quote = self.passive_quotes(
            adjusted_fair, regime, target_position, predicted_edge, effective_fit, volatility,
        )
        position = self.projected_position()
...
class Trader:
    ...
    def load_trader_data(self, trader_data: str) -> Dict[str, Dict[str, List[float]]]:
        empty: Dict[str, Dict[str, List[float]]] = {
            "mid_history": {},
            "spread_history": {},
            "bid_history": {},
            "ask_history": {},
            ...
            if isinstance(raw, dict):
                for product, values in raw.items():
                    if isinstance(values, list):
                        store[product] = [float(v) for v in values[-BaseProductTrader.HISTORY_LENGTH:]]
            cleaned[key] = store
        return cleaned
