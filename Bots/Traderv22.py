...
class BaseProductTrader:
    ...
    def _load_market_state(self) -> None:
        ...
        if getattr(self, "USE_EMA", False):
            alpha = 2.0 / (self.HISTORY_LENGTH + 1)
            ema_key = f"_ema_{self.product}"
            prev_ema = self.mid_history.get(ema_key, self.mid)
            new_ema = alpha * float(self.mid) + (1.0 - alpha) * float(prev_ema)
            self.mid_history[ema_key] = new_ema
            self.recent_average = new_ema
        else:
...
class TomatoesTrader(BaseProductTrader):
    ...
    def take_orders(self, regime: str, target_position: int, adjusted_fair: float) -> Tuple[bool, bool]:
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime) and self.buy_capacity > 0:
            base_limit = self.MAX_TAKE_SIZE + (2 if regime == "trend_up" else 0)
            if regime in {"trend_up", "trend_down"}:
                raw_edge = adjusted_fair - int(self.best_ask)
                edge_ratio = min(1.5, raw_edge / max(self.take_edge("BUY", regime), 0.5))
                take_limit = max(1, int(round(base_limit * edge_ratio)))
            else:
                take_limit = base_limit
        ...
        if int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime) and self.sell_capacity > 0:
            base_limit = self.MAX_TAKE_SIZE + (2 if regime == "trend_down" else 0)
            if regime in {"trend_up", "trend_down"}:
                raw_edge = int(self.best_bid) - adjusted_fair
                edge_ratio = min(1.5, raw_edge / max(self.take_edge("SELL", regime), 0.5))
                take_limit = max(1, int(round(base_limit * edge_ratio)))
            else:
                take_limit = base_limit
            if regime != "mean_revert" and self.projected_position() <= target_position:
...
class Trader:
    ...
    def load_trader_data(self, trader_data: str) -> Dict[str, List[float]]:
        ...
        for key, values in raw_history.items():
            if key.startswith("_ema_"):
                if isinstance(values, (int, float)):
                    cleaned[key] = float(values)
            elif isinstance(values, list):
                cleaned[key] = [float(v) for v in values[-BaseProductTrader.HISTORY_LENGTH :]]
        return cleaned
